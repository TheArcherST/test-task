...
лучшей идеей.  Решил проблему с тем, что FK и транзакции не работали.  Правда
это, насколько понимаю, не поможет в плане обслуживания конкурентных запросов
на аллокацию оператора: SQLite не поддерживает `WITH FOR UPDATE`.  Можно было
бы попробовать добиться нужного поведения силами SQLAlchemy + SQLite, или можно
было бы просто руками сделать лок в процессе который обрабатывает запросы к REST
API, или можно было бы лочить через какой-нибудь Redis... но для всех этих 
решений мне трудно было бы найти обоснование.  Вероятнее всего, если бы такая
задача была в дикой природе, ничего бы не мешало сюда взять вместо SQLite
серверную СУБД, поддерживающую современные техники управления конкурентностью.
